...
export function loadConfig(): Config {
  ...
  'activity',
      ...
      shapeDescriptions: {
        ...
        groupedExecutionStats:
          "Per-activity execution health in ONE row: count, success_count, success_rate, dominant failure_mode, AND the goal-reach numbers (reached_count, graded_count, ungraded_count, reach_rate) over a window — the per-activity_id detail traceAggregateReport (single-metric {key,value}) cannot express. THE way to DETECT a livelock: a family with high count and near-zero success_rate. THE way to ask 'is the substrate reaching its goals': reach_rate, computed from the `reached` verdict over GRADED runs only — it is NOT success_rate (success is exit status; reach is the goal verdict), and it is null when nothing was graded rather than 0. Config: window_hours, limit, min_count, activity_id (restrict to one family — also projects that family's reach numbers to the top level), order (asc|desc by count).",
        activityTemplatesByMetrics:
