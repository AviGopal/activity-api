...
void import("./execution-traces").then((m) => m.insertTraceDigest({ execution_id: executionId, activity_id: activityIdFromRequest, success: validated.success, duration_ms: validated.duration_ms ?? 0, cost_usd: 0, org_id: orgId ?? "public", executed_at: new Date() }, body)).catch(() => {});
  ...
  try {
    ...
    if (want('emission')) {
      for (const m of metrics ?? []) {
        ...
        const rows = rowsByArm.get(String(m['activity_id'])) ?? 0;
        if (rows === 0 && a + b >= 100) {
          violations.push({ invariant: 'emission', activity_id: m['activity_id']!, alpha: a, beta: b, trace_rows: 0, note: 'heavily observed posterior with zero execution rows - the lane is not emitting, or its history was evicted', class: 'anchor_not_found' });
        }
      }
...
app.post('/conservation-audit-emit', async (c) => {
  ...
  try {
    ...
    const auditRes = await fetch(`${ACTIVITY_API_ENDPOINT}/v2/activities/conservation-audit?invariant=all`, {
