  ...
  try {
    ...
    switch (pointer.type) {
        ...
        if (missingTopIds.length > 0) {
          try {
            const topTemplates = await executeAsAuth<any>(
              jwtAuthCtx,
              `SELECT id, name, description, category, task_steps, created_at
               FROM activity
               WHERE record::id(id) IN $ids OR name IN $ids`,
              { ids: missingTopIds },
            );
            templates = [...templates, ...topTemplates];
          } catch {
      ...
      case 'activityTemplate_write': {
        ...
        // Deterministic metadata stamp: read goalSignature directly from writePointer
        ...
        if (
          ...
          if (typeof writePointer.goalSignature === 'string' && writePointer.goalSignature !== '') {
            meta.goalSignature = writePointer.goalSignature;
          }
        }
      ...
      case 'goal_verification_label': {
        ...
        try {
          ...
          const rows = await executeAsAuth<any>(
            jwtAuth,
            `SELECT * FROM goal_verification_labels ${whereClause} ORDER BY created_at DESC LIMIT $limit`,
            bindings,
          );
